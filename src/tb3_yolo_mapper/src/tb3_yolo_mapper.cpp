...
using namespace std::chrono_literals; // required for using the chrono literals such as "ms" or "s"
    ...
    private:
        ...
        int min_cost_;
        bool enable_map_;
        bool enable_global_costmap_;

    public:
        explicit MapLoader(const std::string &name = "tb3_yolo_mapper")
            : rclcpp::Node(name)
        {
            // Parameters
            ...
            this->enable_map_ = this->getParameter_<bool>("enable_map", false).as_bool();
            this->topic_map_ = this->getParameter_<std::string>("topic_map", "map").as_string();
            this->enable_global_costmap_ = this->getParameter_<bool>("enable_global_costmap", true).as_bool();
...
int main(int argc, char *argv[])
{
    ...
    std::shared_ptr<MapLoader> map_loader = std::make_shared<MapLoader>();
